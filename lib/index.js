...
var GitPush = function(options) {
    ...
    // Map of current push operations
    this.pushs = {};
};
...
GitPush.prototype.handle = function (req, res, next) {
    ...
    .then(function() {
        // This request is in the middle of the push
        if (that.pushs[req.repoId]) return;

        return Q(that.prepareRepository(req.repoId))
        .then(function(_repoPath) {
            // Save this path
            that.pushs[req.repoId] = _repoPath
        });
    })
    .then(function() {
        repoPath = that.pushs[req.repoId];
    })
    .then(function() {
